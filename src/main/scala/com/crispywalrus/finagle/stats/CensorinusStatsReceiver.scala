...
import llc.flyingwalrus.censorinus._

/**
  * wrap up using censorinus to forward metrics to either statsd or
  * dogstatsd based on the standard finagle flags.
  *
  ...
  */
class ScalaStatsReceiver extends StatsReceiver {
  ...
  lazy val impl: StatsReceiver = implementation() match {
    ...
    case iname       => throw new IllegalArgumentException(s"$iname is not a valid implementation name")
  }
